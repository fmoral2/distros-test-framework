...
func extractNodesFromState(config *driver.InfraConfig) ([]infraNode, error) {
	stateTfFile := filepath.Join(config.InfraProvisioner.TFNodeSource, "terraform.tfstate.d", config.InfraProvisioner.Workspace, "terraform.tfstate")
	...
	resources.LogLevel("debug", "Extracted nodes: %+v", nodes)
...
func addRoleFromName(name string) string {
	...
	// for now on remote infra the first one is refered as master.
	switch {
