...
func setupAnsibleEnvironment(config *driver.InfraConfig) error {
	...
	if err := runCmdWithTimeout(config.InfraProvisioner.TempDir, 2*time.Minute, "git", "sparse-checkout", "set", ansibleDir); err != nil {
...
func installAnsibleCollection(workingDir string) error {
	...
	if err := runCmdWithTimeout(workingDir, 3*time.Minute, "ansible-galaxy", "collection", "install", "cloud.terraform", "--force"); err != nil {
...
func generateTemplateInventory(config *driver.InfraConfig) error {
	...
	templatePath := filepath.Join(config.InfraProvisioner.TempDir, fmt.Sprintf("ansible/%s/default/inventory-template.yml", config.Product))
...
func ensureSSHAnsibleConfig(config *driver.InfraConfig) error {
		...
		if strings.TrimSpace(line) == "[defaults]" {
			...
			if inDefaultsSection && !sshConfigAdded {
				newLines = append(newLines[:len(newLines)-1],
					fmt.Sprintf("private_key_file = %s", config.Cluster.SSH.PrivKeyPath),
					fmt.Sprintf("remote_user = %s", config.Cluster.SSH.User),
					line)
				sshConfigAdded = true
			}
			inDefaultsSection = false
	...
	if err := os.WriteFile(ansibleCfgPath, []byte(finalConfig), 0644); err != nil {
...
func executeAnsiblePlaybook(config *driver.InfraConfig) error {
	...
	if executeErr := runCmdWithTimeout(config.InfraProvisioner.Ansible.Dir, 25*time.Minute, "ansible-playbook", args...); executeErr != nil {
...
func buildAnsibleArgs(config *driver.InfraConfig, playbookPath string) ([]string, error) {
	...
	args := []string{
		...
		"--extra-vars", fmt.Sprintf("kubeconfig_file=%s", config.InfraProvisioner.KubeconfigPath),
...
func addInstallMethod(args []string, installMethod string) []string {
	...
	args = append(args, "--extra-vars", fmt.Sprintf("install_method=%s", method))
