...
func (p *Provider) provisionInfrastructure(cfg *driver.InfraConfig) (*driver.Cluster, error) {
...
func (p *Provider) destroyInfrastructure(product, module string) (string, error) {
	...
	if workspace == "" {
		...
		return "", fmt.Errorf("no workspace specified for qainfra destroy")
	}
	...
	if resources.IsRunningInContainer() {
		...
		nodeSource = fmt.Sprintf("/tmp/qainfra-tofu-%s", workspace)
	} else {
		_, callerFilePath, _, _ := runtime.Caller(0)
		rootDir = filepath.Join(filepath.Dir(callerFilePath), "..")
		nodeSource = os.Getenv("TERRAFORM_NODE_SOURCE")
		if nodeSource == "" {
			nodeSource = filepath.Join(rootDir, "tmp", fmt.Sprintf("qainfra-tofu-%s", workspace))
		}
	}

	if err := runCmdWithTimeout(nodeSource, 5*time.Minute, "tofu", "destroy", "-auto-approve", "-var-file=vars.tfvars"); err != nil {
